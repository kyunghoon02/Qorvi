...
if [[ "$MODE" == "env-only" ]]; then
  ...
  if [[ "$block_count" -gt 0 ]]; then
    echo "Next: fix BLOCK env values before beta open."
    exit 1
  fi
  echo "Next: run corepack pnpm beta:prep && corepack pnpm beta:evidence"
  exit 0
fi
...
echo
echo "Launch summary"
echo "PASS  Ready for beta open"
if [[ "$warn_count" -gt 0 ]]; then
  echo "WARN  Optional values remain unset; review beta release package before opening."
fi
echo "Next:"
echo "- Review docs/runbooks/beta-launch-review.md"
echo "- Review docs/runbooks/beta-open-prep.md"
