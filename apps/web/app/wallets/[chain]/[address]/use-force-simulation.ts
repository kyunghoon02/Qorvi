    ...
    const simulate = () => {
      ...
      for (let i = 0; i < simNodes.length; i++) {
        for (let j = i + 1; j < simNodes.length; j++) {
          ...
          if (distSq < 1500000) {
            ...
            if (!a.dragging) { a.vx += (dx / dist) * f; a.vy += (dy / dist) * f; }
            if (!b.dragging) { b.vx -= (dx / dist) * f; b.vy -= (dy / dist) * f; }
          }
        }
      }
      ...
      edges.forEach((edge) => {
        ...
        if (source && target) {
          ...
          if (!source.dragging) { source.vx += (dx / dist) * f; source.vy += (dy / dist) * f; }
          if (!target.dragging) { target.vx -= (dx / dist) * f; target.vy -= (dy / dist) * f; }
        }
      });
