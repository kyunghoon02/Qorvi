...
func alertDeliveryRetryBatchLimitFromEnv() int {
	value := strings.TrimSpace(os.Getenv("WHALEGRAPH_ALERT_DELIVERY_RETRY_BATCH_LIMIT"))
	if value == "" {
		return 25
	}
...
func alertDeliveryRetryLimitFromEnv() int {
	value := strings.TrimSpace(os.Getenv("WHALEGRAPH_ALERT_DELIVERY_RETRY_LIMIT"))
	if value == "" {
		return 3
	}
	parsed, err := strconv.Atoi(value)
...
func alertDeliveryRetryBaseDelayFromEnv() time.Duration {
	value := strings.TrimSpace(os.Getenv("WHALEGRAPH_ALERT_DELIVERY_RETRY_BASE_DELAY_SECONDS"))
	if value == "" {
		return time.Minute
	}
	parsed, err := strconv.Atoi(value)
