...
1. `AI findings foundation`
   - 목표: findings generation, AI brief, entity interpretation을 제품의 1차 서빙 객체로 고정
2. `signal-first product surfaces`
   - 목표: home/discover, signal feed, wallet brief, entity page를 그래프보다 앞세운 서빙 구조 완성
3. `beta open hardening`
   - 목표: findings-first 경험을 기준으로 target environment preflight와 operator sign-off를 다시 정리

## 0.3 현재 Immediate Next

바로 다음 구현 우선순위는 아래 3개로 고정한다.

1. `findings generation baseline`: candidate finding, evidence bundle, AI summary contract를 제품 파이프라인에 연결
2. `AI Findings Feed + AI Wallet Brief`: 검색형 흐름보다 findings/brief가 먼저 보이는 서빙 경로 구현
3. `Entity Interpretation + signal-first APIs`: `/findings`, `/wallet/.../brief`, `/entity/:id` 중심 API와 화면 정리
...
- 다음은 interpretation finding rule을 `label context`에서 `flow/path/evidence` 중심으로 더 고도화하고, analyst drill-down payload에 `historical outcome scoring`과 richer evidence references(tx/path/entity refs)를 붙이는 단계
...
5. findings-first API와 내부 운영 흐름이 beta 수준으로 안정화된다.
