...
- `[O]` local compose Neo4j 포트 충돌 회피 정리: FlowIntel local Neo4j는 `localhost:8687` 기준으로 고정
...
이 문서는 [FlowIntel PRD](/Users/kh/Github/FlowIntel/FlowIntel_PRD_v2_0.md)를 실제 개발 가능한 작업 단위로 분해한 실행 계획서다.
목표는 `production launch`에 필요한 범위를 명확히 고정하고, 각 단계별 산출물과 완료 기준을 정의하는 것이다.

FlowIntel은 단순한 지갑 조회 앱이 아니라, 온체인 활동의 의미를 설명하는 AI analyst product로 구현한다. 중심 엔진은 다음과 같다.
...
FlowIntel의 핵심 저장 구조와 재처리 가능한 ingest 골격을 만든다.
...
FlowIntel의 첫 버전은 “모든 데이터를 다 보여주는 플랫폼”이 아니라,
“제한된 예산 안에서 고래 행동의 의미를 빠르게 포착해주는 인텔리전스 제품”으로 완성되어야 한다.
## AI Workspace

- Dedicated AI work now lives under `/Users/kh/Github/FlowIntel/flowintel-ai`
